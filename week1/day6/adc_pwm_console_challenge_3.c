...
int main(void) {
    ...
    static uint8_t actual_duty = 0;
    static uint32_t last_fade_time = 0;
    static uint8_t target_duty = 0;      /* ← ADD THIS */
    static bool alarm_was_active = false; /* ← MOVE HERE */
    ...
    while (1) {
        ...
        /* ← FADE CHECK: Every loop iteration ← */
        if (now - last_fade_time >= 20) {
            if (actual_duty < target_duty) {
                actual_duty += 5;
                if (actual_duty > 255) actual_duty = 255;
            }

            if (actual_duty > target_duty) {
                actual_duty -= 5;
                if (actual_duty < 0) actual_duty = 0;
            }
            last_fade_time = now;
        }
        pwm_set(actual_duty);  /* Always use actual_duty */
        /* ← END FADE ← */
        ...
        if (auto_report && (now - last_report >= (uint32_t)report_rate * 100UL)) {
            ...
            /* Update target duty (fade will pull actual toward this) */
            target_duty = (uint8_t)map_value(raw, 0, 1023, 0, 255);

            /* Alarm check */
            bool alarm_is_now_active = (raw > 716);

            if (alarm_is_now_active && !alarm_was_active) {
                uart_send_string("[ALARM] Voltage exceeded 3.5V threshold!\r\n");
            }

            if (!alarm_is_now_active && alarm_was_active) {
                uart_send_string("[CLEAR] Voltage back in range\r\n");
            }

            alarm_was_active = alarm_is_now_active;
            ...
            uart_send_uint16(actual_duty);  /* Print actual, not target */
