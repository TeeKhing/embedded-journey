...
int main(void) {
    ...
    while (1) {
        ...
        if (auto_report && (now - last_report >= (uint32_t)report_rate * 100UL)) {
            ...
            /* Alarm state (add this outside the loop, in main() with other statics) */
            static bool alarm_was_active = false;

            /* Inside the auto-report block, after calculating raw: */
            bool alarm_is_now_active = (raw > 716);  // 3.5V threshold

            if (alarm_is_now_active && !alarm_was_active) {
                /* Transition: just crossed into alarm */
                uart_send_string("[ALARM] Voltage exceeded 3.5V threshold!\r\n");
            }

            if (!alarm_is_now_active && alarm_was_active) {
                /* Transition: just left alarm */
                uart_send_string("[CLEAR] Voltage back in range\r\n");
            }

            alarm_was_active = alarm_is_now_active;  /* Remember for next time */
